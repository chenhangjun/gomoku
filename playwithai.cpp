...
#include "ui_playwithai.h"
#include <cmath>
#include <QPainter>
#include <QSound>
#include "mainwindow.h"

PlayWithAI::PlayWithAI(QWidget *parent) :
    QMainWindow(parent),
...
void PlayWithAI::paintEvent(QPaintEvent *)  //绘制棋盘
{
    QPainter p(this);
...
void PlayWithAI::mouseReleaseEvent(QMouseEvent *e)
{
    if(flag == 0) {    //未选棋色或先后手
        return;
    }
...
//改变鼠标状态
void PlayWithAI::mouseMoveEvent(QMouseEvent *event)
{
...
//计算价值
void PlayWithAI::Score(int a, int b)
{
...
bool PlayWithAI::isLegal(int a,int b)
{
...
//左上-右下
void PlayWithAI::dir1(int newx, int newy)
{
    int cnt1 = 0, cnt2 = 0;    //该点两侧连子数
    int flag1 = 0, flag2 = 0;   //标记两侧是否是死子

    if(isLegal(newx - 1, newy - 1)) {
        if(chessboard[newx - 1][newy -1] != 0) {
            cnt1++;   //此点有子
        }
    } else {
        flag1 = 1;    //越界，死子
...
//右上-左下
void PlayWithAI::dir2(int newx, int newy)
{
    int cnt1 = 0, cnt2 = 0;    //该点两侧连子数
    int flag1 = 0, flag2 = 0;   //标记两侧是否是死子
...
// 上-下
void PlayWithAI::dir3(int newx, int newy)
{
    int cnt1 = 0, cnt2 = 0;    //该点两侧连子数
    int flag1 = 0, flag2 = 0;   //标记两侧是否是死子

    if(isLegal(newx, newy - 1)) {
        if(chessboard[newx][newy -1] != 0) {
            cnt1++;   //此点有子
        }
    } else {
        flag1 = 1;    //越界，死子
...
//左-右
void PlayWithAI::dir4(int newx, int newy)
{
    int cnt1 = 0, cnt2 = 0;    //该点两侧连子数
    int flag1 = 0, flag2 = 0;   //标记两侧是否是死子

    if(isLegal(newx - 1, newy)) {
        if(chessboard[newx - 1][newy] != 0) {
            cnt1++;   //此点有子
        }
    } else {
        flag1 = 1;    //越界，死子
...
//判断是否五子连棋
bool PlayWithAI::JudgeWin(int x, int y)
{
    int cnt = 1;
    firx = x;
    firy = y;
